...
class UserSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        existen_users = User.objects.filter(Q(username=username) | Q(email=email))
        if len(existen_users) > 0:
            raise serializers.ValidationError(u"Ya existe un usuario con ese nombre de usuario y/o email registrado")
...
class PhotoListSerializer(PhotoSerializer):
    ...
    """
    Si creamos el Meta, para decirle que herede del anterior hay que decirselo
    Con esto, personalizamos los datos que queremos que devuelva solo ciertos campos
    """
