...
class HomeView(View):
    ...
    def get(self, request):
        ...
        """
        all_photos = Photo.objects.filter(visibility=Photo.VISIBILITY_PUBLIC).order_by('-created_at') # recupera todas las fotos de la DB ordenadas desc por created_at
...
class LoginView(View):
    ...
    def get(self, request):
        login_form = LoginForm() # Formulario vacío cuando la petición es GET
    ...
    def post(self, request):
        ...
        if login_form.is_valid():
            ...
            if user is None: #si es null
                ...
                if user.is_active: #si está activo hacemos el login
                    login(request, user)
...
class PhotoList(ListView):
    """
    Lista de fotos del sistema
    """
    model = Photo
    template_name = 'photos/list.html' #en /photos/
